...
results/raw/                    all 72 published condition outputs
results/combined/               canonical 1,800-trial pilot outputs
...
`preexisting_results_unchanged` warning records unrelated files changing in
the parent benchmark while the original broader campaign ran; all 1,800
trials in this scoped ACBBA/HIPC/PI release completed successfully.
...
Inspect the exact 72-condition, 1,800-run matrix without writing files:
...
ACBBA and HIPC cells contain the completion-retention rerun, while PI retains
its original results. The exact scenario, raw replacement outputs,
36-condition manifest, communication settings, and seed rule are included.
...
run(fullfile('<clone>', 'analysis', 'build_plan_depth_paper_figures.m'))
```
